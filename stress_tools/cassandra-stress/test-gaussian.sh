...
CS_PARAMS()
{
	...
	echo "no-warmup n=$num_itarations -node $SCYLLA_HOST -rate threads=$nthreads $RATE_LIMIT -mode native cql3 connectionsPerHost=$CONN_PER_HOST -pop dist=gaussian\($min..$max,$mean,$GAUS_STDEV\) -schema keyspace=ks$ks_id replication\(strategy=NetworkTopologyStrategy, datacenter1=1\)"
}
...
{
        local iterations=$1
        echo "Write test with UNIFORM distribution. $iterations iterations..."
        local i
        local itn
        local loader
        local ld
